...
sleep_time = 60
sleep_on_inverter_read_error = 120
...
def get_data():
    ...
    for key, val in registers.items():
        print("Try to read inverter")
        try:
            res = modbus.read_holding_registers(register_addr=val['id'], quantity=1)
        except pysolarmanv5.pysolarmanv5.NoSocketAvailableError:
            print("pysolarmanv5.pysolarmanv5.NoSocketAvailableError: Sleeping for {sleep_on_inverter_read_error} seconds".format(sleep_on_inverter_read_error=sleep_on_inverter_read_error))
        ...
        res_human = res[0] * scale + offset
        do_round = val.get('do_round', False)
        if do_round:
            res_human = round(res_human)

        output[key] = res_human
        print('{key}: {res_human} {units}'.format(key=key, res_human=res_human, units=val['units']))
    ...
    print('MQTT Message:\n{}'.format(message))
    send_by_mqtt('homeassistant/sensor/inverter/state', message)
